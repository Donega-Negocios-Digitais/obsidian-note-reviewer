...
> Historical normalization note (2026-02-12): this plan keeps original deployment intent and adds current environment security guidance.
...
Security reminder:
- Share `.env.example` only; never share a real `.env` file with secrets over WhatsApp.
- `SUPABASE_SERVICE_ROLE_KEY` must stay server-side and out of frontend env files.
