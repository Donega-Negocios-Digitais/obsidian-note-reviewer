-- Fix relink ordering bug in ensure_public_user_linked.
-- Previous implementation could fail when moving legacy user references:
--   update ... set user_id = <new_auth_id> where user_id = <legacy_id>
-- before a public.users row existed for <new_auth_id>.
-- This migration creates a placeholder row first, then rewires FKs, then removes legacy row.
...
BEGIN
  ...
  LOOP
    ...
    -- Avoid unique collisions on (note_id, user_id) before rewiring legacy references.
